...
function Customers() {
  ...
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 w-full h-[calc(100vh-5rem)]">
          <div className="px-5 lg:px-32 py-10  bg-slate-50 border border-slate-900 text-center flex flex-col justify-center items-center">
          ...
          <div className="px-4 lg:px-20 border bg-slate-500  border-slate-900 text-center flex flex-col justify-center items-center">
          

            <h2 className='uppercase text-center text-white text-2xl font-bold p-5'>Customer List</h2>
            <ul class="w-full text-sm font-normal lg:font-medium  rounded-lg bg-slate-100 border-gray-600 text-slate-500">
              <li className="w-full font-bold text-slate-700 grid text-xs lg:text-base grid-cols-[10%_34%_20%_12%_12%_12%] lg:grid-cols-[10%_30%_20%_18%_12%_10%] p-2 border-gray-400 border-b-2 rounded-t-lg ">
                  <span>Name</span> 
                  <span>Email</span>
                  <span>Phone</span>
                  <span>Age</span></li>
              {customers?.map(c => (
                <li className="w-full grid text-xs lg:text-base grid-cols-[10%_34%_20%_12%_12%_12%] lg:grid-cols-[10%_30%_20%_18%_12%_10%] p-2 border-gray-400 border-b-2 rounded-t-lg " key={c?._id}>
                  <span>{c?.name}</span> 
                  <span>{c?.contactInfo?.email}</span>
                  <span>{c?.contactInfo?.phone}</span>
                  <span>{c?.contactInfo?.age}</span>

                 <button onClick={() => editCustomer(c)} className=' bg-yellow-500  text-white text-xs lg:text-base p-1 lg:px-2 mr-1 lg:mr-2 rounded-sm'>Update</button>
                    <button onClick={() => deleteCustomer(c._id)} className=' bg-red-600 text-white text-xs lg:text-base p-1 lg:px-2 rounded-sm'>Delete</button>
